...
class SimpleInvoicePreviewScreen extends StatefulWidget {
  const SimpleInvoicePreviewScreen({super.key});

  @override
  State<SimpleInvoicePreviewScreen> createState() =>
      _SimpleInvoicePreviewScreenState();
}

class _SimpleInvoicePreviewScreenState
    extends State<SimpleInvoicePreviewScreen> {
  ...
  Widget _buildPdfViewer(ERPInvoice inv) {
    ...
    // Vista previa del PDF más grande
    return Container(
  ...
  Future<void> _downloadPdf(ERPInvoice inv) async {
    ...
    final fileName =
        'Factura_${inv.numeroFactura.isNotEmpty ? inv.numeroFactura : 'DOC'}.pdf';
  ...
  Future<void> _printPdf(ERPInvoice inv) async {
    ...
    try {
      final fileName =
          'Factura_${inv.numeroFactura.isNotEmpty ? inv.numeroFactura : 'DOC'}';
  ...
  void _viewFullscreen(ERPInvoice inv) {
    ...
    PdfViewerService.showPdfFullscreen(
      pdfBytes: pdfBytes!,
      title: 'Factura_${inv.numeroFactura}',
    );
  }
